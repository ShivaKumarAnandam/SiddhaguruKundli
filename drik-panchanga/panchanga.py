...
def sunrise(jd, place):
  ...
  lat, lon, tz = place
  result = swe.rise_trans(tjdut=jd - tz/24, body=swe.SUN, geopos=(lon, lat, 0), rsmi=swe.BIT_DISC_CENTER + swe.CALC_RISE)
...
def sunset(jd, place):
  ...
  lat, lon, tz = place
  result = swe.rise_trans(tjdut=jd - tz/24, body=swe.SUN, geopos=(lon, lat, 0), rsmi=swe.BIT_DISC_CENTER + swe.CALC_SET)
...
def moonrise(jd, place):
  ...
  lat, lon, tz = place
  result = swe.rise_trans(tjdut=jd - tz/24, body=swe.MOON, geopos=(lon, lat, 0), rsmi=swe.BIT_DISC_CENTER + swe.CALC_RISE)
  rise = result[1][0]  # julian-day number
  # Convert to local time
...
def moonset(jd, place):
  ...
  lat, lon, tz = place
  result = swe.rise_trans(tjdut=jd - tz/24, body=swe.MOON, geopos=(lon, lat, 0), rsmi=swe.BIT_DISC_CENTER + swe.CALC_SET)
  setting = result[1][0]  # julian-day number
  # Convert to local time
...
def karana(jd, place):
  ...
  # 5. Check for additional karanas (there can be 2-3 karanas in a day)
  # Each karana is 6 degrees, so check if we can fit more
  moon_phase_tmrw = (lunar_longitude(rise + 1) - solar_longitude(rise + 1)) % 360
  tomorrow = ceil(moon_phase_tmrw / 6)
  
  # Calculate how many karanas occur between sunrise today and tomorrow
  karanas_today = int((tomorrow - today) % 60)
  if karanas_today >= 2:
    # Second karana
    ...
    if karanas_today >= 3:
      # Third karana
      leap_karana2 = ((today % 60) + 2)
      if leap_karana2 > 60:
        leap_karana2 = leap_karana2 - 60
